...
int main() {
    try {
        ...
        svr.set_exception_handler([](const httplib::Request &req, httplib::Response &res, std::exception_ptr ep) {
            std::string tracking_id = generateErrorTrackingId();
            std::string error_message = "Unknown Exception";
            ...
            try {
                ...
                // Extract standard exception message
                error_message = e.what();
            }

            // Log error to Console & error.log
            logErrorToConsoleAndFile((tracking_id), 500, req.path, req.method, error_message);

            // Return isolated error response based on API request type
            bool is_api_request = req.path.rfind("/api", 0) == 0;
            if (is_api_request) {
                std::string json_res = R"({"status":"error", "message":"Internal server error", "tracking_id":")" + tracking_id + R"("})";
                res.status = 500;
                res.set_content(json_res, "application/json; charset=utf-8");
            } else {
                std::string html = readFileToString("./public/error.html");
                if (html.empty()) {
                    res.status = 500;
                    res.set_content("Internal Server Error. Tracking ID: " + tracking_id, "text/plain; charset=utf-8");
                    return;
                }
                html = replacePlaceholder(html, "{{TITLE}}", "Internal Server Error (500)");
                html = replacePlaceholder(html, "{{DESCRIPTION}}", "An unexpected error occurred on the server.");
                html = replacePlaceholder(html, "{{TRACKING_ID}}", tracking_id);
                res.status = 500;
                res.set_content(html, "text/html; charset=utf-8");
            }
        ...
        std::jthread gc_thread([&session_manager, &login_limiter](std::stop_token stoken) {
            while (!stoken.stop_requested()) {
                // checking the stop token every 1 second
        ...
        // Print log when standard exception occurs
        std::cerr << "Fatal Exception: " << e.what() << std::endl;
        return 1;
    } catch (...) {
        // Print log when unknown exception occurs
