...
| **`957b697`** | [main.cpp](../src/main.cpp), `run.ps1` | **Chore (THREAT-06-Followup 완료)** | `main.cpp` OS 조건부 레거시 코드 정리 마무리 — `a147d15`에서 `limitProcessMemory()`를 제거하고도 남아있던 미사용 `#include <sys/resource.h>` 삭제(line 20). 더 이상 쓰이지 않던 MSVC 네이티브 빌드용 `run.ps1`은 삭제 대신 Docker 전용 전체 리셋+재빌드 스크립트(`docker compose down` → `build --no-cache` → `up`, DB 볼륨은 보존)로 용도 전환 — `docker compose ps`/`logs`로 app·mariadb 정상 기동까지 실증 확인 |
| **`Pending`** | `Ochlos/*`, `ScreeningRouter/*`, `CMakeLists.txt` | **Feat/Security (New-Firewall Phase 1)** | ① `Ochlos/` C++ 기반 다중 연결 점유(DoS Starvation) 시뮬레이션 도구(`tcp_syn_flooding.cpp`) 작성. ② L7 웹 애플리케이션 앞단에서 OSI 3계층(IP) 및 4계층(Port) 패킷/연결 요청을 실시간으로 감시 및 로깅하는 스크리닝 라우터(`ScreeningRouter/main.cpp`) 구축 및 8080 포트 실측 타격 연동 검증 완료. |
