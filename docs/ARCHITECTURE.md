...
## 0. 개발 및 실습 인프라 환경 구성 (Dev & Lab Infrastructure)

이 프로젝트는 **로컬 호스트 OS**, **Dev Container(개발 작업실)**, 그리고 **도커 런타임 서비스군(실제 서비스)**의 3개 계층으로 물리적/논리적 환경이 엄격히 분리되어 운용됩니다.

```
┌──────────────────────────────────────────────────────────────────────────────────┐
│ [1. 로컬 호스트 PC] (macOS MacBook Air / Windows)                                   │
│  - 역할: Docker 엔진 구동, 브라우저 웹 접속(localhost:8080), 호스트 터미널 운용       │
│  - 명령어: docker compose up/down/exec/logs, curl 테스트                           │
└────────────────────────┬─────────────────────────────────────────────────────────┘
                         │ Docker 엔진 가상화
       ┌─────────────────┴──────────────────────────────────────────┐
       │                                                            │
┌──────▼────────────────────────────┐    ┌──────────────────────────▼──────────────┐
│ [2. Dev Container (작업실)]       │    │ [3. 도커 서비스 컨테이너군 (3계층 망 분리)]  │
│ - 환경: Bartimaeus Dev Container  │    │  (1_external_net / 2_dmz_net / 3_internal) │
│ - 경로: /workspaces/Bartimaeus_app│    │                                         │
│ - 도구: clangd, C++20, CMake, Ninja│    │  ┌───────────────────────────────────┐  │
│ - 특징: 내부 docker CLI 부재      │    │  │ 🛡️ bartimaeus-screening-router    │  │
│ - 명령어: 코드 편집, 컴파일 데이터베이스 │    │  │ (10.10.0.2 / 10.20.0.2 / L3-L4)   │  │
└───────────────────────────────────┘    │  └─────────────────┬─────────────────┘  │
                                         │                    │                     │
┌───────────────────────────────────┐    │  ┌─────────────────▼─────────────────┐  │
│ [4. Ochlos (외부 공격자 C2)]       │    │  │ 🔄 bartimaeus-reverse-proxy       │  │
│ - 역할: 독립된 외부 Linux 공격 서버 │    │  │ (10.20.0.3:8080 / L7 Proxy)       │  │
│ - 실증: Raw Packet SYN/ICMP Flooding│   │  └─────────────────┬─────────────────┘  │
└───────────────────────────────────┘    │                    │                     │
                                         │  ┌─────────────────▼─────────────────┐  │
                                         │  │ 🌐 bartimaeus-app (:9090 Web)     │  │
                                         │  │ 🗄️ bartimaeus-mariadb (:3306 DB)  │  │
                                         │  └───────────────────────────────────┘  │
                                         └─────────────────────────────────────────┘
```

### [중요] 명령어 실행 위치 매트릭스 (Command Execution Matrix)

| 구분 | 실행 위치 (터미널) | 주요 실행 대상 명령어 | 이유 및 주의사항 |
| :--- | :--- | :--- | :--- |
| **인프라 제어** | **호스트 PC 터미널**<br>(Mac Terminal / Win PowerShell) | `docker compose up --build`<br>`docker compose down`<br>`docker compose exec ...`<br>`docker compose logs ...` | Dev Container 내부에는 `docker` CLI가 없으므로 반드시 호스트 OS 터미널에서 실행해야 함. |
| **외부 통신 검증** | **호스트 PC 터미널**<br>(또는 호스트 웹 브라우저) | `curl -i http://localhost:8080/`<br>웹 브라우저 접속 | 외부 클라이언트 입장에서 스크리닝 라우터로 인입되는 실제 트래픽 검증용. |
| **코드 개발 / 빌드** | **Dev Container 터미널**<br>(Antigravity IDE 통합 터미널) | `cmake`, `ninja`, `git status`<br>소스코드 검사 및 디버깅 스크립트 | C++ 컴파일러(Clang/GCC)와 라이브러리가 완전히 세팅된 전용 리눅스 빌드 환경. |
